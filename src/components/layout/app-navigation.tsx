...
export type WorkspaceView = "dashboard" | "family" | "goals" | "activities" | "checklist" | "calendar" | "reports" | "settings";

const primaryNavigation: Array<{ label: string; href: string; view: WorkspaceView; icon: typeof Home; enabled: boolean }> = [
  { label: "Dashboard", href: "/dashboard", view: "dashboard", icon: Home, enabled: true },
  { label: "Family", href: "/family", view: "family", icon: UsersRound, enabled: true },
  { label: "Goals", href: "/goals", view: "goals", icon: Target, enabled: true },
  { label: "Activities", href: "/activities", view: "activities", icon: ListChecks, enabled: true },
  { label: "Checklist", href: "/checklist", view: "checklist", icon: CheckSquare, enabled: true },
  { label: "Calendar", href: "/calendar", view: "calendar", icon: CalendarDays, enabled: true },
  { label: "Reports", href: "/reports", view: "reports", icon: BarChart3, enabled: true },
  { label: "Settings", href: "/settings", view: "settings", icon: Settings, enabled: true },
];
...
function NavigationItem({
  ...
  const isActive = activeView ? activeView === item.view : pathname === item.href || pathname.startsWith(`${item.href}/`);
  ...
  if (onViewChange) {
    return (
      <button className={`${baseClass} ${stateClass}`} onClick={() => onViewChange(item.view)} type="button">
        <Icon aria-hidden="true" className="h-4 w-4" />
        <span>{item.label}</span>
      </button>
    );
...
export function AppSidebar({ activeView, email, onViewChange }: { email?: string } & WorkspaceNavigationProps) {
  return (
    <aside className="hidden min-h-screen w-72 shrink-0 border-r border-sky-100 bg-white px-4 py-5 dark:border-neutral-800 dark:bg-neutral-950 lg:flex lg:flex-col">
      ...
      <nav aria-label="Main navigation" className="mt-8 flex flex-1 flex-col gap-1">
        {primaryNavigation.map((item) => (
          <NavigationItem activeView={activeView} item={item} key={item.label} onViewChange={onViewChange} />
        ))}
      </nav>
    </aside>
  );
...
export function MobileBottomNav({ activeView, onViewChange }: WorkspaceNavigationProps) {
  return (
    <nav
    ...
    >
      {mobileNavigation.map((item) => (
        <NavigationItem activeView={activeView} compact item={item} key={item.label} onViewChange={onViewChange} />
      ))}
    </nav>
  );
}
